...
class _DosenIzinViewState extends State<DosenIzinView> {
  ...
  Future<void> _load() async {
    ...
    if (mounted) setState(() {
      _loading = false;
      if (res['success'] == true) _izins = res['data']['izins'] ?? [];
  ...
  Widget build(BuildContext context) {
    ...
    if (_izins.isEmpty) return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      const SizedBox(height: 16),
      const Text('Tidak Ada Pengajuan Izin', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
      const SizedBox(height: 8),
      const Text('Semua pengajuan izin sudah diproses.', style: TextStyle(color: AppColors.neutral)),
    ]));
        ...
        itemBuilder: (context, i) {
          ...
          return Container(
            ...
            child: Column(children: [
              Padding(
                ...
                child: Row(children: [
                  CircleAvatar(
                    ...
                    child: Text(
                      (user['name'] ?? 'M').toString().isNotEmpty ? user['name'].toString()[0].toUpperCase() : 'M',
                      style: const TextStyle(fontWeight: FontWeight.w700, color: AppColors.primary),
                    ),
                  ),
                  const SizedBox(width: 12),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(user['name'] ?? 'Mahasiswa', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                    const SizedBox(height: 2),
                    Text('${matkul['nama'] ?? '-'} • ${iz['tanggal'] ?? '-'}', style: const TextStyle(fontSize: 12, color: AppColors.neutral)),
                  ])),
                  Container(
              ...
              if (iz['alasan'] != null) Container(
                width: double.infinity,
                margin: const EdgeInsets.fromLTRB(16, 0, 16, 12),
                padding: const EdgeInsets.all(12),
                decoration: BoxDecoration(color: isDark ? AppColors.surfaceVariantDark : AppColors.backgroundLight, borderRadius: BorderRadius.circular(10)),
                child: Text(iz['alasan'] ?? '', style: const TextStyle(fontSize: 13, color: AppColors.neutral)),
              ),
